...
public class AasRegistryService : IAasRegistryService
{
    ...
    public AssetAdministrationShellDescriptor CreateAasDescriptorFromDB(AASSet aasDB) => GlobalCreateAasDescriptorFromDB(aasDB);
    ...
    public static AssetAdministrationShellDescriptor GlobalCreateAasDescriptorFromDB(AASSet aasDB)
    {
        ...
        using (AasContext db = new AasContext())
        {
            ...
            e.ProtocolInformation.Href = $"{externalRepository}/shells/{Base64UrlEncoder.Encode(ad.Id)}";
            Console.WriteLine($"AAS {ad.IdShort} {e.ProtocolInformation.Href}");
            e.Interface         = "AAS-1.0";
            ad.Endpoints        = new List<Endpoint> {e};
            ...
            {
                var sd  = new SubmodelDescriptor {IdShort = submodelDB.IdShort, Id = submodelDB.Identifier ?? string.Empty};
                var esm = new Models.Endpoint();

                esm.ProtocolInformation      = new ProtocolInformation();
                esm.ProtocolInformation.Href = $"{AasxServer.Program.externalRepository}/shells/{Base64UrlEncoder.Encode(ad.Id)}/submodels/{Base64UrlEncoder.Encode(sd.Id)}";
                esm.Interface                = "SUBMODEL-1.0";
                sd.Endpoints                 = new List<Models.Endpoint> {esm};
                                                                                               ...
                                                                                               string? aasIdentifier = null)
    {
        ...
        if (aasDescriptors != null && aasDescriptors.Count != 0)
        {
            foreach (var ad in aasDescriptors)
            {
                ...
                if (aasIdentifier != null && aasIdentifier.Equals(ad.Id, StringComparison.Ordinal))
                {
                    found = true;
                }
        ...
        if (aasRegistry is {SubmodelElements: not null})
        {
            ...
            foreach (var sme in aasRegistry.SubmodelElements)
            {
                ...
                var found = (aasIdentifier == null && assetList.IsNullOrEmpty()) || (aasIdentifier != null && aasID != "" && descriptorJSON != "" && aasIdentifier.Equals(aasID, StringComparison.Ordinal));

                if (!assetList.IsNullOrEmpty() && assetID != "" && descriptorJSON != "" && assetList.Contains(assetID))
                {
                    found = true;
                }
